...
//  uint8_t zcl_RegisteredMsgTaskID = TASK_NO_TASK;
//#endif
//
...
//
//// ZCL Sequence number
//uint8_t zcl_SeqNum = 0x00;
//
//uint8_t zcl_TransID = 0;  // This is the unique message ID (counter)
//
//static uint8_t savedZCLTransSeqNum = 0;
//
///*********************************************************************
// * EXTERNAL VARIABLES
// */
...
//#endif
//
//static zclAttrRecsList *zclFindAttrRecsList( uint8_t endpoint );
...
//    osal_msg_send( zcl_RegisteredMsgTaskID, (uint8_t *)pCmd );
//  }
//
//  return ( TRUE );
//}
...
// */
//uint8_t zcl_getParsedTransSeqNum( void )
//{
...
// * @return  ZSuccess if OK
// */
//ZStatus_t zcl_SendCommand( uint8_t srcEP, afAddrType_t *destAddr,
...
//    {
//      *pBuf++ = LO_uint16_t( readCmd->attrID[i] );
//      *pBuf++ = HI_uint16_t( readCmd->attrID[i] );
//    }
//
...
//      zclReadRspStatus_t *statusRec = &(readRspCmd->attrList[i]);
//
//      *pBuf++ = LO_uint16_t( statusRec->attrID );
//      *pBuf++ = HI_uint16_t( statusRec->attrID );
...
//      *pBuf++ = LO_uint16_t( writeRspCmd->attrList[i].attrID );
//      *pBuf++ = HI_uint16_t( writeRspCmd->attrList[i].attrID );
//    }
//
...
//      *pBuf++ = LO_uint16_t( reportRec->attrID );
//      *pBuf++ = HI_uint16_t( reportRec->attrID );
//
//      if ( reportRec->direction == ZCL_SEND_ATTR_REPORTS )
//      {
//        *pBuf++ = reportRec->dataType;
//        *pBuf++ = LO_uint16_t( reportRec->minReportInt );
//        *pBuf++ = HI_uint16_t( reportRec->minReportInt );
//        *pBuf++ = LO_uint16_t( reportRec->maxReportInt );
//        *pBuf++ = HI_uint16_t( reportRec->maxReportInt );
//
//        if ( zclAnalogDataType( reportRec->dataType ) )
//        {
...
//      else
//      {
//        *pBuf++ = LO_uint16_t( reportRec->timeoutPeriod );
//        *pBuf++ = HI_uint16_t( reportRec->timeoutPeriod );
//      }
//    } // for loop
//
...
// * @return  ZSuccess if OK
// */
//ZStatus_t zcl_SendConfigReportRspCmd( uint8_t srcEP, afAddrType_t *dstAddr,
...
//      *pBuf++ = LO_uint16_t( cfgReportRspCmd->attrList[i].attrID );
//      *pBuf++ = HI_uint16_t( cfgReportRspCmd->attrList[i].attrID );
//    }
//
//    // If there's only a single status record and its status field is set to
//    // SUCCESS then omit the attribute ID field.
...
//      *pBuf++ = LO_uint16_t( readReportCfgCmd->attrList[i].attrID );
//      *pBuf++ = HI_uint16_t( readReportCfgCmd->attrList[i].attrID );
//    }
//
...
// * @return  ZSuccess if OK
// */
//ZStatus_t zcl_SendReadReportCfgRspCmd( uint8_t srcEP, afAddrType_t *dstAddr,
...
//      *pBuf++ = LO_uint16_t( reportRspRec->attrID );
//      *pBuf++ = HI_uint16_t( reportRspRec->attrID );
//
//      if ( reportRspRec->status == ZCL_STATUS_SUCCESS )
//      {
//        if ( reportRspRec->direction == ZCL_SEND_ATTR_REPORTS )
...
//          *pBuf++ = LO_uint16_t( reportRspRec->minReportInt );
//          *pBuf++ = HI_uint16_t( reportRspRec->minReportInt );
//          *pBuf++ = LO_uint16_t( reportRspRec->maxReportInt );
//          *pBuf++ = HI_uint16_t( reportRspRec->maxReportInt );
//
//          if ( zclAnalogDataType( reportRspRec->dataType ) )
//          {
...
//        else
//        {
//          *pBuf++ = LO_uint16_t( reportRspRec->timeoutPeriod );
//          *pBuf++ = HI_uint16_t( reportRspRec->timeoutPeriod );
//        }
//      }
//    }
//
...
// * @return  ZSuccess if OK
// */
//ZStatus_t zcl_SendReportCmd( uint8_t srcEP, afAddrType_t *dstAddr,
...
//    // Load the buffer - serially
//    uint8_t *pBuf = buf;
//    *pBuf++ = LO_uint16_t(pDiscoverCmd->startAttr);
//    *pBuf++ = HI_uint16_t(pDiscoverCmd->startAttr);
...
//    {
//      *pBuf++ = LO_uint16_t(pDiscoverRspCmd->attrList[i].attrID);
//      *pBuf++ = HI_uint16_t(pDiscoverRspCmd->attrList[i].attrID);
...
//
//  // Load the buffer - serially
//  buf[0] = LO_uint16_t(pDiscoverAttrsExt->startAttr);
//  buf[1] = HI_uint16_t(pDiscoverAttrsExt->startAttr);
...
//    for ( i = 0; i < pDiscoverRspCmd->numAttr; i++ )
//    {
//      *pBuf++ = LO_uint16_t(pDiscoverRspCmd->aExtAttrInfo[i].attrID);
//      *pBuf++ = HI_uint16_t(pDiscoverRspCmd->aExtAttrInfo[i].attrID);
		 ...
		 ( zclCmdTable[inMsg.hdr.commandID].pfnParseInProfile != NULL )*/) {
			...
			if (inMsg.attrCmd != NULL) {
				...
				for (int i = 0; i < readRspCmd->numAttr; i++) {
					zclReadRspStatus_t *statusRec = &(readRspCmd->attrList[i]);
			  printf("cluster ID = %04X, attribute ID = %04X, dataType = %02X, dataLen = %02X, data: ",pkt->cluster_id, statusRec->attrID,statusRec->dataType, statusRec->dataLen);
			for(int j =0;j<statusRec->dataLen;j++){
				printf("%02X  ", statusRec->data[j]);
			}
				printf("\n");
					...
					str_data -= sizeof(zclOutgoingMsg_t);
//    		  uint8_t * tempData = (uint8_t*)&outgoingMsg;
//    		  for(int j=0; j<sizeof(zclOutgoingMsg_t);j++){
...
//    		  printf("\n");

					{
						...
						set_data_dynamic_msg(s_msg, (uint8_t*) &str_data[0], len);
...
//      printf("inMsg.hdr.commandID = %02x \n", inMsg.hdr.commandID);
...
//  if ( hdr->fc.manuSpecific )
//  {
//    *pData++ = LO_uint16_t( hdr->manuCode );
//    *pData++ = HI_uint16_t( hdr->manuCode );
//  }
//
...
// */
//uint8_t zclGetClusterOption( uint8_t endpoint, uint16_t clusterID )
//{
//  uint8_t option;
...
// */
//ZStatus_t zclUpdateClusterOption( uint8_t endpoint, uint16_t clusterID, uint8_t optionMask, uint8_t optionValue )
//{
//  zclOptionRec_t *pOption;
//
//  pOption = zclFindClusterOption( endpoint, clusterID );
...
//      *buf++ = LO_uint16_t( *((uint16_t*)attrData) );
//      *buf++ = HI_uint16_t( *((uint16_t*)attrData) );
//      break;
//
...
//      pStr = (uint8_t*)attrData;
//      len = BUILD_uint16_t( pStr[0], pStr[1] );
...
//      zclWriteRec_t *statusRec = &(writeCmd->attrList[i]);
//
//      statusRec->attrID = BUILD_uint16_t( pBuf[0], pBuf[1] );
//      pBuf += 2;
//      statusRec->dataType = *pBuf++;
//
//      attrDataLen = zclGetAttrDataLength( statusRec->dataType, pBuf );
...
static void *zclParseInWriteRspCmd(zclParseCmd_t *pCmd) {
	...
	writeRspCmd = (zclWriteRspCmd_t *) malloc(sizeof(zclWriteRspCmd_t) + pCmd->dataLen);
	if (writeRspCmd != NULL) {
		if (pCmd->dataLen == 1) {
...
//      reportRec->attrID = BUILD_uint16_t( pBuf[0], pBuf[1] );
//      pBuf += 2;
//      if ( reportRec->direction == ZCL_SEND_ATTR_REPORTS )
//      {
//        // Attribute to be reported
//        reportRec->dataType = *pBuf++;
//        reportRec->minReportInt = BUILD_uint16_t( pBuf[0], pBuf[1] );
//        pBuf += 2;
//        reportRec->maxReportInt = BUILD_uint16_t( pBuf[0], pBuf[1] );
//        pBuf += 2;
//
//        // For attributes of 'discrete' data types this field is omitted
//        if ( zclAnalogDataType( reportRec->dataType ) )
...
//        reportRec->timeoutPeriod = BUILD_uint16_t( pBuf[0], pBuf[1] );
//        pBuf += 2;
//      }
//    } // while loop
//  }
...
//      cfgReportRspCmd->attrList[i].attrID = BUILD_uint16_t( pBuf[0], pBuf[1] );
//      pBuf += 2;
//    }
//  }
//
...
//      readReportCfgCmd->attrList[i].attrID = BUILD_uint16_t( pBuf[0], pBuf[1] );
//      pBuf += 2;
//    }
//  }
//
...
//      reportRspRec->attrID = BUILD_uint16_t( pBuf[0], pBuf[1] );
//      pBuf += 2;
//
//      if ( reportRspRec->status == ZCL_STATUS_SUCCESS )
//      {
...
//          reportRspRec->minReportInt = BUILD_uint16_t( pBuf[0], pBuf[1] );
//          pBuf += 2;
//          reportRspRec->maxReportInt = BUILD_uint16_t( pBuf[0], pBuf[1] );
//          pBuf += 2;
//
//          if ( zclAnalogDataType( reportRspRec->dataType ) )
//          {
...
//        else
//        {
//          reportRspRec->timeoutPeriod = BUILD_uint16_t( pBuf[0], pBuf[1] );
//          pBuf += 2;
//        }
//      }
//    }
...
//  {
//    pDiscoverCmd->startAttr = BUILD_uint16_t( pBuf[0], pBuf[1] );
//    pBuf += 2;
...
//    for ( i = 0; i < numAttr; i++ )
//    {
//      pDiscoverRspCmd->attrList[i].attrID = BUILD_uint16_t( pBuf[0], pBuf[1] );
//      pBuf += 2;
...
//  uint8_t *pBuf = pCmd->pData;
//  uint8_t i;
//  uint8_t numCmds = ZCLDISCRSPCMD_DATALEN(pCmd->dataLen);  // determine length of command ID variable array
//
...
//    {
//      pDiscoverRspCmd->aExtAttrInfo[i].attrID = BUILD_uint16_t( pBuf[0], pBuf[1] );
//      pBuf += 2;
